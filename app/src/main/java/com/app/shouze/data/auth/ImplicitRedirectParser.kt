...
object ImplicitRedirectParser {
    ...
    sealed interface Result {
        ...
        /** User denied the app or AniList returned an error fragment. */
        data class Denied(val description: String) : Result
    ...
    fun parse(uriString: String): Result {
        ...
        if (error != null) {
            val description = fragmentParams["error_description"] ?: queryParams["error_description"] ?: error
            return Result.Denied(description)
        }
        ...
        val token = fragmentParams["access_token"] ?: queryParams["access_token"]
            ?: return if (fragmentParams.containsKey("token_type") || queryParams.containsKey("token_type")) {
                Result.Denied("AniList did not return an access token.")
            } else {
                Result.NotAnAuthRedirect
            }

        if (token.isBlank()) return Result.Denied("AniList returned an empty access token.")
